  ...
  return (
    <section className="mt-16 pt-12 border-t border-stone-200/80 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        {/* Card 3: Partner pilot — črna postane samo, ko je aktivna (klik na kartico ali gumb) */}
        <div
          onClick={() => setActiveDarkCard('partner')}
          className={`border rounded-3xl p-6 flex flex-col justify-between shadow-xs transition-all cursor-pointer ${
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span
                className={`text-xs font-bold uppercase tracking-wider ${
                  activeDarkCard === 'partner' ? 'text-slate-400' : 'text-slate-500'
                }`}
              >
                PARTNERSTVO
              </span>
            ...
            <div>
              <h3 className={`text-xl font-bold ${activeDarkCard === 'partner' ? 'text-white' : 'text-slate-900'}`}>
                Partner
              </h3>
              <div className="mt-2 flex items-baseline gap-1">
                <span className={`text-3xl font-black ${activeDarkCard === 'partner' ? 'text-white' : 'text-slate-900'}`}>
                  Po dogovoru
                </span>
              </div>
              <p className={`text-xs mt-1 ${activeDarkCard === 'partner' ? 'text-slate-400' : 'text-slate-500'}`}>
            ...
            <ul
              className={`space-y-2.5 text-xs pt-3 border-t ${
                activeDarkCard === 'partner' ? 'text-slate-300 border-slate-700' : 'text-slate-600 border-slate-100'
              }`}
            >
              <li className="flex items-center gap-2">
                <Check className={`w-4 h-4 shrink-0 ${activeDarkCard === 'partner' ? 'text-emerald-400' : 'text-emerald-600'}`} />
        ...
        {/* Card 5: Poslovno — črna postane samo, ko je aktivna (klik na kartico ali gumb) */}
        <div
          onClick={() => setActiveDarkCard('business')}
          className={`border rounded-3xl p-6 flex flex-col justify-between shadow-xs transition-all cursor-pointer ${
            activeDarkCard === 'business'
              ? 'bg-slate-900 border-slate-900'
              : 'bg-white border-slate-200/90 hover:border-slate-300'
          }`}
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className={`text-xs font-bold uppercase tracking-wider ${activeDarkCard === 'business' ? 'text-slate-400' : 'text-slate-500'}`}>
                ZA PODJETJA
              </span>
            </div>

            <div>
              <h3 className={`text-xl font-bold ${activeDarkCard === 'business' ? 'text-white' : 'text-slate-900'}`}>Poslovno</h3>
              <div className="mt-2 flex items-baseline gap-1">
                <span className={`text-3xl font-black ${activeDarkCard === 'business' ? 'text-white' : 'text-slate-900'}`}>Po dogovoru</span>
              </div>
              <p className={`text-xs mt-1 ${activeDarkCard === 'business' ? 'text-slate-400' : 'text-slate-500'}`}>
                Prilagojen paket za organizacije z večjim številom uporabnikov.
              </p>
            </div>

            <ul className={`space-y-2.5 text-xs pt-3 border-t ${activeDarkCard === 'business' ? 'text-slate-300 border-slate-700' : 'text-slate-600 border-slate-100'}`}>
              <li className="flex items-center gap-2">
                <Check className={`w-4 h-4 shrink-0 ${activeDarkCard === 'business' ? 'text-emerald-400' : 'text-emerald-600'}`} />
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setActiveDarkCard('business');
              setInterestTier('business');
            }}
            className={`w-full mt-6 py-2.5 px-4 rounded-xl text-xs font-semibold transition-all flex items-center justify-center gap-1.5 text-center cursor-pointer ${
              activeDarkCard === 'business'
                ? 'bg-white hover:bg-slate-100 text-slate-900'
